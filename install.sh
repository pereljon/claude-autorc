...
if [[ ! -f "$CONFIG_FILE" ]]; then
    ...
    # LaunchAgent disabled by default unless --enable-launchagent was passed
...
# Enable/disable batch startup via LaunchAgent (-a/--all).
# When false, the LaunchAgent runs but exits without starting sessions.
# Default: true (but installer sets to false unless --enable-launchagent is used)
${launchagent_line}
CONFIG_EOF
else
