...
每个项目的 `.claude/settings.local.json` 中有一个 Claude Code Stop hook，在每次对话轮次后调用 `claude-mux --tipotd`。该命令检查今天是否已经显示过提示（通过 `~/.claude-mux/.tip-date`）。如果是，约 6ms 内退出。如果不是，显示一条提示并记录今天的日期。

提示默认启用（`TIP_OF_DAY=true`）。在任何会话内用"启用提示"或"禁用提示"来切换。`TIP_MODE=daily` 全天显示同一条提示；`TIP_MODE=random` 每次调用时随机选择一条提示（使用 Stop hook 时，由于每日门控，这意味着每天一条随机提示）。

`--tip` 命令始终有效，不受每日门控影响，所以你随时可以说"tip"。
